...
Read this when a phase or resume returns `PLAN_READY`, `WORK_DONE`, or
`LAND_READY`. These gates are the only context-cut approval protocol.
...
After PLAN_READY or WORK_DONE approval, perform the transition before the
context cut. Then commit only the task records before the context cut. Ask the
user to run `/clear`, then `/flow <id>`, and end the turn.
