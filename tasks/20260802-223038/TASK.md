...
- [x] On master, before sprouting: commit the already-staged-in-worktree tatr
...
- [x] `home/modules/scripts/afk.sh`: replace `flow_step()` with a generic
...
- [x] `afk.sh`: add `phase_label()` (`$1` id) returning what a human should
...
- [x] `afk.sh` `report_phase()`: print `phase_label` and its gloss, and set
      `SPIN_PHASE` from the same label.
- [x] `afk.sh`: replace `require_step()` with `require_activity()` (`$1` id,
      ...
      actually says.
- [x] `afk.sh` `lifecycle_gate()`: take the precondition as an activity and
...
- [x] `afk.sh` `cmd_run()`: `LAND_READY` requires `RESOLUTION: DONE` instead
...
- [x] `afk.sh` `fingerprint()`: hash the activity, the gates and the
      resolution instead of the single step, so a session that only earned a
      gate still counts as durable progress.
- [x] `afk.sh`: update the gate labels passed to `gate()` so they stay
...
- [x] `home/modules/agents/skills/flow/gates.md`: the `PLAN_READY` row becomes
...
- [x] `flow/SKILL.md`: rewrite the route table in v1 vocabulary - no activity
      ...
      budget.
- [x] `flow/resume.md`: dispatch from ACTIVITY plus GATES, not FLOW STEP; the
...
- [x] `flow/epic.md`: `flow <epic-id> --to DONE` becomes walking the epic to
      closure with `tatr -r <task-root> flow <epic-id>`; mention `tatr close
      --resolution WONTDO|SUPERSEDED --reason ...` for retiring a child.
- [x] `work/SKILL.md` step 2: drop `tatr flow <id> --to WORKING` - the
...
- [x] `work/review-feedback.md`: the REVIEWING -> WORKING move becomes `tatr
      -r <task-root> rewind <id> --to WORKING`, and the handoff back becomes
      `tatr -r <task-root> flow <id>`.
- [x] `review/SKILL.md` and `review/rounds.md`: the APPROVE handoff becomes
...
- [x] `compound/SKILL.md`: the finish becomes `tatr -r <task-root> flow <id>`,
      which earns `RETRO` and closes the task as `DONE` in one motion.
- [x] `plan/SKILL.md` step 5: the approval records itself through `tatr -r
      <task-root> flow <id>`; planning still leaves the lifecycle alone.
- [x] `home/modules/agents/skills/check.sh` rule 8: replace the retired
...
- [x] `check.sh` rule 9: add `rewind` and `close` to `TATR_ID_SUBS`, so the
      new backward move and the retire path are held to `-r <task-root>` like
      every other task-taking call.
- [x] `home/modules/scripts/afk-test.sh` fixtures: rewrite the four side
...
- [x] `afk-test.sh` expectations: the phase lines assert the new labels
      ...
      fails the run.
- [x] Sweep for stragglers: `grep -rn 'FLOW STEP\|PLAN STATUS\|--to \(PLANNED\|WORKING\|REVIEWING\|COMPOUNDING\|DONE\|DROPPED\)' home/modules/agents/skills home/modules/scripts` must be empty, then run the three checks in the DoD.
...
What and why: every consumer of tatr's lifecycle in this repository now speaks
v1. `afk.sh` reads `ACTIVITY`, `GATES` and `RESOLUTION` through one generic
`task_field()`, reports a task as `phase_label()` (activity plus earned gates,
`WORKING+PLAN`; the resolution once closed; `OPEN` before it starts), and
asserts the durable half of each gate: `PLAN_READY` asserts the `PLAN` gate,
`WORK_DONE` asserts the `REVIEWING` cursor, `LAND_READY` asserts
`RESOLUTION: DONE`. The flow-family skills drop `--to`, name `tatr rewind` for
the backward move and `tatr close --resolution` for the retire path, and
`check.sh` rule 8 guards value-anchored v1 marker names while rule 9 holds
`rewind` and `close` to `-r <task-root>`.

Alternatives: see DECISION.md. The two load-bearing ones are asserting the
`PLAN` gate rather than the `WORKING` cursor (a plan gate that half-succeeds on
a blocked dependency still earned `PLAN`), and printing a closed record's
RESOLUTION verbatim instead of the literal `DONE` the plan named, so a `WONTDO`
record cannot report as finished work.

Difficulties: the three skill files that changed most (`flow/SKILL.md`,
`work/SKILL.md`, `plan/SKILL.md`) all broke their check.sh word budgets on the
first pass - v1 needs more vocabulary than v0 to say the same thing. Resolved
by compressing the route table cells and the transition prose rather than by
raising a budget.

Evidence: `bash home/modules/scripts/afk-test.sh` passes 15, fails 0 (baseline
1 passed, 14 failed). `bash home/modules/agents/skills/check.sh` clean, and a
planted `set ACTIVITY: WORKING by hand` clause was confirmed to trip rule 8
before being reverted. The straggler grep is empty. `nix flake check` passes
all 6 checks.

Reflection: the fixtures were the honest part of the migration - rewriting them
forced the v1 edge semantics (which transitions earn a gate, which earn
nothing) to be stated exactly once, in `lifecycle_gate`'s postcondition kind,
