...
- [ ] plan SKILL.md: DoD format - each item ends with its proof:
...
- [ ] work SKILL.md verify step: run each `test:`/`cmd:` proof explicitly;
      `manual:` items are reported as pending user confirmation, never
      self-ticked.
- [ ] review SKILL.md: the reviewer executes the `test:`/`cmd:` proofs and
      lists open `manual:` items next to the verdict.
- [ ] flow SKILL.md: `manual:` items do not block landing; they accumulate on
      the umbrella GOAL.md's Manual acceptance section and are presented as
      a batch at Finish (the user-acceptance gate).
- [ ] tatr SKILL.md task-format section: mirror the proof notation.
